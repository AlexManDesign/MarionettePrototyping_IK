...
export class FirstPersonCamera extends cc.Component {
    ...
    public start () {
        ...
        if (this.input) {
            ...
            if (useMouseInput()) {
                input.on(cc.Input.EventType.MOUSE_DOWN, this._onMouseDown, this);
                input.on(cc.Input.EventType.MOUSE_MOVE, this._onMouseMove, this);
                input.on(cc.Input.EventType.MOUSE_UP, this._onMouseUp, this);
                input.on(cc.Input.EventType.MOUSE_WHEEL, this._onMouseWheel, this);
            } else {
                input.on(cc.Input.EventType.TOUCH_START, this._onTouchBegin, this);
                input.on(cc.Input.EventType.TOUCH_MOVE, this._onTouchMove, this);
                input.on(cc.Input.EventType.TOUCH_END, this._onTouchEnd, this);
            }
        }
    }
    ...
    public update (deltaTime: number) {
        ...
        const targetHorizonPositionChanged = 
            !cc.math.approx(targetLastPosition.x, targetWorldPosition.x, 1e-5)
            || !cc.math.approx(targetLastPosition.z, targetWorldPosition.z, 1e-5);
        const targetVerticalPositionChanged =
            !cc.math.approx(targetLastPosition.y, targetWorldPosition.y, 1e-5)

        if (targetHorizonPositionChanged && this.autoTraceEnabled) {
